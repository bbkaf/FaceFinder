...
class PreviewView: UIView {
    ...
    private func createLayer(with rect: CGRect) -> CAShapeLayer {
        ...
        mask.opacity = 0.75
        mask.borderColor = UIColor.orange.cgColor
    ...
    func drawFaceBoundingBox(faceObservation: VNFaceObservation) {
        _ = createLayer(with: normalizeDimensions(with: faceObservation))
    }
    
    func normalizeDimensions(with faceObservation: VNFaceObservation) -> CGRect {
    ...
    func drawFaceWithLandmarks(faceObservation: VNFaceObservation) {
        let faceBounds = normalizeDimensions(with: faceObservation)
        let faceLayer = createLayer(with: faceBounds)
        
//        drawFaceLandmarks(on: faceLayer, faceLandmarkRegion: (faceObservation.landmarks?.nose)!, closePath: false)
